...
class Sudoku(object):
    ...
    def solve(self, isInitial=True, emptyIndex=0):
        ...
        self.reorder_by_least_constraining_value(chosen_tile)
    ...
    def reorder_by_least_constraining_value(self, chosen_tile):
        row, col = chosen_tile
        dom_vals = self.domains[chosen_tile]
        square_top_left_row = (row // 3) * 3
        square_top_left_col = (col // 3) * 3
        val_constraint_map = {}
        for val in dom_vals:
            ...
            for i in range(9):
                current = (row, i)
                if current != chosen_tile and val in self.domains[current]:
                    num_constraints += 1
                current = (i, col)
                if current != chosen_tile and val in self.domains[current]:
                    num_constraints += 1
            for i in range(square_top_left_row, square_top_left_row + 3):
                for j in range(square_top_left_col, square_top_left_col + 3):
                    current = (i, j)
                    if current != chosen_tile and i != row and j != col and val in self.domains[current]:
                        num_constraints += 1
            val_constraint_map[val] = num_constraints
        self.domains[chosen_tile] = sorted(dom_vals, key=lambda val: val_constraint_map[val])
